...
MatrixXd PrefixSpan::get_NonZeroWeightX()
{
	uint tNonZeroWeightSize = get_NonZeroWeightSize();
	MatrixXd tOutX = MatrixXd::Zero(mN, tNonZeroWeightSize);
	for (uint i = 0, m = 0; i < mActive.size(); i++)
	{
		if (mActive[i]->w != 0)
		{
			for (uint n = 0; n < mActive[i]->x.size(); n++)
			{
				tOutX(mActive[i]->x[n], m)++;
			}
			m++;
		}
	}
...
uint PrefixSpan::get_NonZeroWeightSize()
{
	int tCnt = 0;
...
VectorXd PrefixSpan::get_weight()
{
	uint tNonZeroWeightSize = get_NonZeroWeightSize();
	VectorXd tWeights = VectorXd::Zero(tNonZeroWeightSize);
	for (uint i = 0, idx = 0; i < mActive.size(); i++)
	{
		if (mActive[i]->w != 0)
		{
			tWeights(idx) = mActive[i]->w;
			idx++;
		}
	}
	return tWeights;
...
void PrefixSpan::printTree(string aFilename, vector<double> aPvalues)
{
	ofstream tFile(aFilename);
	tFile << "pattern,supportSum,addλ,w,p-value" << '\n';
	int tCnt = 0;
	for (uint j = 0; j < mActive.size(); j++)
	{
		if(mActive[j]->w != 0){
			tFile << mActive[j]->patternStr << "," << mActive[j]->supportSum << "," << mActive[j]->addLambda << "," << mActive[j]->w << "," << aPvalues[tCnt] << endl;
			tCnt++;
		}
	}
}
